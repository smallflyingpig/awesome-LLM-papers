...
这是一个可直接阅读、编辑和重新编译的中文、英文及中英双语 LaTeX 技术报告项目。报告系统梳理 Code Agent 从代码基座训练到可执行软件工程智能体的完整构建路径，并区分模型能力、Agent harness、执行环境、评估系统与 test-time compute 的作用边界。
...
- `code-agent-construction-report.pdf`：最终编译版 PDF，共 23 页。
- `code-agent-construction-report-en.tex` / `.pdf`：完整英文版源码与 PDF，共 28 页；公式、引用键、数据 schema 和图表结构与中文版保持一致。
- `code-agent-construction-report-bilingual.tex` / `.pdf`：英文在前、中文在后的双语自包含版本，共 52 页；两种语言使用独立目录、计数器和内部链接命名空间。
...
- 目录后的全局符号表，统一解释工具契约、episode、训练、长上下文、RL 和评估记号，并消歧重载符号；
...
tectonic --outdir build code-agent-construction-report-en.tex
tectonic --outdir build code-agent-construction-report-bilingual.tex
```
...
latexmk -xelatex -interaction=nonstopmode code-agent-construction-report-en.tex
latexmk -xelatex -interaction=nonstopmode code-agent-construction-report-bilingual.tex
```
