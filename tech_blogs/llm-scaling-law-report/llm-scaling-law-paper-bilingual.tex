...
Kaplan et al. placed language-model parameters $N$, training tokens $D$, and training compute $C$ in a unified framework and fitted loss curves across several orders of magnitude\cite{en:kaplan2020}. Their representative exponents were approximately $\alpha_N=\textbf{0.076}$, $\alpha_D=\textbf{0.095}$, and $\alpha_C=\textbf{0.050}$. They reached an early compute-optimal conclusion: as the budget grows, model size should increase faster than data volume, approximately as
\begin{equation}
  N_{\mathrm{opt}}\propto C^{0.73},\qquad
  D_{\mathrm{opt}}\propto C^{0.27}.
  \label{en:eq:kaplan-opt}
\end{equation}
This conclusion affects the GPT-3-style "large model, relatively few tokens, early stop" training route \cite{en:brown2020}. Its contribution is not to give the eternal index, but to turn "whether the expanded model is effective" into an engineering problem of "use the pilot to predict the target run."

\subsection{Why power laws work but are not laws of nature}

Power laws can arise from variance limits, resolution limits, or approximation error in different regimes. Theory can explain common functional shapes but does not yield universal exponents across data distributions and architectures\cite{en:bahri2021}. Empirical curves may also exhibit plateaus, double descent, delayed transitions, or slope changes; a smoothly broken power law can therefore fit better than a single power law\cite{en:caballero2022}. Equation~\eqref{en:eq:single-power} should be treated as \term{a local model over a bounded scale range under a fixed training recipe}.
...
Table~\ref{en:tab:symbols} defines the notation and measurement conventions used in this report. For a standard dense decoder-only Transformer, a common first-order approximation is
\begin{equation}
  C\approx 6ND.
  \label{en:eq:compute}
\end{equation}
...
Hoffmann et al. trained more than 400 models with 70M--16B parameters and 5B--500B tokens, using the joint loss surface \cite{en:hoffmann2022}:
\begin{equation}
  L(N,D)=E+\frac{A}{N^{\alpha}}+\frac{B}{D^{\beta}},
  \qquad A,B,\alpha,\beta>0.
  \label{en:eq:chinchilla}
\end{equation}
Under the calculation constraints of the formula ~\eqref{en:eq:compute}, let $D=C/(6N)$, substitute the formula ~\eqref{en:eq:chinchilla}:
\begin{equation}
  L(N\mid C)=E+A N^{-\alpha}
  +B\left(\frac{6N}{C}\right)^{\beta}.
  \label{en:eq:constrained-loss}
\end{equation}
Let the derivative of ~\eqref{en:eq:constrained-loss} with respect to $N$ be zero, we can get the balance of the two marginal benefits, and get
\begin{align}
  N_{\mathrm{opt}}(C)
...
\label{en:tab:kaplan-chinchilla}
...
Henighan et al. extended power-law analysis to images, video, multimodal modeling, and mathematical problem generation\cite{en:henighan2020}. Hernandez et al. interpreted pretraining gains as an effective increase in downstream data and characterized transfer scaling\cite{en:hernandez2021}. The GPT-4 technical report illustrates a production use case: small models trained with roughly four orders of magnitude less compute predicted final internal codebase loss, while models trained with roughly three orders of magnitude less compute predicted performance on a subset of HumanEval. The report also states that some capabilities remained difficult to predict\cite{en:openai2023gpt4}. This evidence supports \term{predictability within a controlled training system}, not universal prediction of arbitrary capabilities.

\subsection{From Smooth Loss to the "Emergence" Debate}

"Emergent abilities" interprets the benchmark ability of the small model to be almost zero and the large model to suddenly increase as the transition \cite{en:wei2022}. But the discontinuous indicator itself can create jumps: after replacing the exact match with token-level accuracy or Brier score, many curves will be smoother \cite{en:schaeffer2023}. Therefore, it is necessary to at least distinguish: the internal capabilities of the model may grow smoothly; the task success rate may be S-shaped due to the threshold; prompts and sampling create high variance; three model points are not enough to prove phase transition.

\subsection{From More Data to Effective Data}

Data-constrained scaling research shows that modest duplication of data may result in little loss under a fixed computing budget, but the marginal value of duplication eventually approaches zero \cite{en:muennighoff2023}. DeepSeek LLM reports that different data versions will change the optimal allocation index, indicating that the scaling coefficient itself can be used as an in-system diagnostic quantity for data and recipes \cite{en:deepseekllm2024}. DataComp-LM uses standardized small model experiments to show that the filtering strategy can significantly move the performance frontier \cite{en:datacomplm2024}; the data mixing law also uses the mixture proportion as a fitable variable \cite{en:datamixing2024}.
...
\begin{equation}
  ...
  \label{en:eq:lifecycle}
\end{equation}
Instead of just minimizing training FLOPs. The result of inference-aware scaling is: \textbf{ Under high call volume, training a smaller model that has seen more tokens may have a lower total cost }\cite{en:sardana2024}. Test-time scaling further shows that \textit{ adaptive allocation search or verifier compute can be more effective on some problems than simply expanding parameters }\cite{en:snell2024}. Modern scaling thus extends from the $N$--$D$--$C_{\mathrm{train}}$ triangle to the joint optimization of training, deployment and inference time computation.
...
Appendix ~\ref{en:app:model-inventory} contains 32 public model families in 2020--2026. The main text only extracts comparisons suitable for forming judgments. In table ~\ref{en:tab:dense-families}, GPT-3, Pythia, and Cerebras-GPT size points are densely populated; LLaMA, Llama 3.1, Qwen2.5/Qwen3, and OLMo 2 are more representative of modern data-rich and deployment-oriented training. The cross-generation public figures of LongCat, Kimi and GLM provide a newer longitudinal comparison of MoE; the parameters and training tokens of Claude Fable 5 are not disclosed and can only be used to mark the evidence boundaries of the closed source frontier.

\begin{table}[htbp]
\centering
\small
\caption{Representative families of dense models and their evidence bounds.}
\label{en:tab:dense-families}
\begin{tabularx}{\textwidth}{p{2.2cm}p{3.9cm}p{2.5cm}Y}
\toprule
Family & Size gradient & Training token & Supported judgment \\
\midrule
GPT-3 & 125M--175B, 8 points in total & About 300B each & Size/few-shot trend of the same recipe; not suitable for estimating compute-optimal ratio \cite{en:brown2020} \\
Pythia & 70M--12B, 8 points in total & Each about 300B & Open checkpoints and fixed data order, suitable for training dynamics \cite{en:biderman2023} \\
Cerebras-GPT & 111M--13B, 7 points in total & 20 tokens/param. & Public Chinchilla-style family\cite{en:cerebrasgpt2023} \\
LLaMA & 7B, 13B, 33B, 65B & 1.0T/1.4T & data-rich small model can exceed the old large model; different token budget \cite{en:llama2023} \\
Llama 3.1 & 8B, 70B, 405B & exceeds 15T & Modern family far exceeds 20 tokens/parameter; cannot be connected to the old generation \cite{en:llama31} \\
Qwen2.5 & 0.5B--72B, 7 points in total & 18T & Modern dense size gradient, description data/recipe mobile frontier\cite{en:qwen25} \\
Qwen3 Dense & 0.6B--32B, 6 points in total & About 36T & The new generation small model catching up with the old generation large model is the overall movement of the curve, not parameter failure \cite{en:qwen3} \\
OLMo 2 & 1B, 7B, 13B, 32B & up to 5T--6T & Data, code, recipes and checkpoints auditable \cite{en:olmo2} \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
Mixtral 8x7B & 46.7B & 12.9B & Undisclosed & Storage capacity is nearly 47B, single token calculation is nearly 13B\cite{en:mixtral2024} \\
DeepSeek-V2 & 236B & 21B & 8.1T & Large total capacity and low active compute\cite{en:deepseekv2} \\
DeepSeek-V3 & 671B & 37B & 14.8T & 671B cannot be directly substituted into the dense formula ~\eqref{en:eq:compute}\cite{en:deepseekv3} \\
Qwen3-30B-A3B & 30B & 3B & About 36T & Small active model uses expert capacity to improve the effect \cite{en:qwen3} \\
Qwen3-235B-A22B & 235B & 22B & About 36T & total/active Determine capacity and cost together \cite{en:qwen3} \\
Llama 4 Scout/Maverick & 109B/400B & 17B/17B & Undisclosed & Multimodal mixed with distillation, not clean scaling suite\cite{en:llama4} \\
DeepSeek-V4 Flash/Pro & 284B/1.6T & 13B/49B & Unpublished & 2026 endpoint, only supports architectural trend observation \cite{en:deepseekv4} \\
\bottomrule
\end{tabularx}
\end{table}
...
\clearpage
\section{Glossary}
\label{en:app:glossary}

\begin{table}[htbp]
\centering
\caption{Scaling law Common term. }\label{en:tab:glossary}
...
\captionof{table}{2020-- will disclose the LLM family, size, training token and scaling evidence in 2026. }\label{en:tab:model-inventory}
\end{minipage}
...
\bottomrule
\endlastfoot
2020 & GPT-3\cite{en:brown2020} & Dense & 125M; 350M; 760M; 1.3B; 2.7B; 6.7B; 13B; 175B & Same as total & is about 300B each & A & Same family parameter scaling and few-shot trends; compute-optimal ratio cannot be estimated. \\
2021 & Gopher\cite{en:rae2021} & Dense & 44M; 117M; 417M; 1.4B; 7.1B; 280B & Same as total & Main model about 300B & A$-$ & Size scaling has uneven benefits from different tasks. \\
2022 & Chinchilla\cite{en:hoffmann2022} & Dense & 70B & Same as total & 1.4T & A & Same calculation comparison and compute-optimal with Gopher allocation. \\
2022 & PaLM\cite{en:palm2022} & Dense & 8B; 62B; 540B & Same as total & 780B & A$-$ & Size trends and apparent benchmark transitions within the family. \\
2022 & OPT\cite{en:opt2022} & Dense & 125M; 350M; 1.3B; 2.7B; 6.7B; 13B; 30B; 66B; 175B & Same as total & main setting is about 180B & B$+$ & has wide size coverage; it must be combined with recipe and loss control. \\
2022 & BLOOM\cite{en:bloom2022} & Dense & 560M; 1.1B; 1.7B; 3B; 7.1B; 176B & Same as total & 176B Model approx. 350B & B & Multilingual scale viewing; size gradient notched. \\
2023 & Pythia\cite{en:biderman2023} & Dense & 70M; 160M; 410M; 1B; 1.4B; 2.8B; 6.9B; 12B & Same as total & Approximately 300B each & A$+$ & One of the best publicly controlled suites suitable for training dynamics. \\
2023 & Cerebras-GPT\cite{en:cerebrasgpt2023} & Dense & 111M; 256M; 590M; 1.3B; 2.7B; 6.7B; 13B & Same as total & 20 tokens/parameter & A & Public Chinchilla-style family. \\
2023 & LLaMA\cite{en:llama2023} & Dense & 7B; 13B; 33B; 65B & Same as total & 1.0T or 1.4T & B$+$ & data-rich The small model can outperform the old large model; it is not a uniaxial fit. \\
2023 & Llama 2\cite{en:llama2} & Dense & 7B; 13B; 70B & Same as total & 2T & B$+$ & Deployment gradient of the same generation; the number of points is not enough to fit the index robustly. \\
2023 & Falcon\cite{en:falcon2023} & Dense & 7B; 40B; 180B & Same as total & 180B Up to about 3.5T & B & data-rich and web filtering; token budget changes with size. \\
2023 & DeepSeek LLM\cite{en:deepseekllm2024} & Dense & 7B; 67B & Same as total & 2T & A$-$ & pilot extrapolates and shows that data versions change the fit index. \\
2023 & Mixtral 8x7B\cite{en:mixtral2024} & MoE & 46.7B & 12.9B & Undisclosed & C$+$ & total and active compute must be separated; not scaling fit. \\
2024 & Open\allowbreak ELM\cite{en:openelm2024} & Dense & 270M; 450M; 1.1B; 3B & Same as total & 1.5T & B$+$ & Comparison of small model architecture with token budget. \\
2024 & OLMo\cite{en:olmo2024} & Dense & 1B; 7B & Same as total & About 2T--3T & B$+$ & Data sequence, checkpoints and training logs are auditable. \\
2024 & Llama 3.1\cite{en:llama31} & Dense & 8B; 70B; 405B & Same as total & More than 15T & B$+$ & Modern data-rich family; does not support universal coefficients. \\
2024 & Qwen2.5\cite{en:qwen25} & Dense & 0.5B; 1.5B; 3B; 7B; 14B; 32B; 72B & Same as total & 18T & B$+$ & Modern size gradient showing data/recipe mobile frontier. \\
2024 & Qwen2.5-Coder\cite{en:qwen25coder} & Dense & 0.5B; 1.5B; 3B; 7B; 14B; 32B & Same as total & Continued training corpus 5.5T & B$+$ & Code domain gradient; base pretraining and CPT need to be distinguished. \\
2024 & Gemma 1\cite{en:gemma1} & Dense & 2B; 7B & Same as total & 2T; 6T & B & Two points; it means that the deployment family does not fix tokens/parameters. \\
2024 & Gemma 2\cite{en:gemma2} & Dense & 2B; 9B; 27B & Same as total & 2T; 8T; 13T & C$+$ & 2B/9B contains distillation. The counterexample shows that you cannot just look at the parameters. \\
2024 & DeepSeek-V2\cite{en:deepseekv2} & MoE & 236B & 21B & 8.1T & B & Separate storage capacity and per-token compute. \\
2024 & DeepSeek-V3\cite{en:deepseekv3} & MoE & 671B & 37B & 14.8T & B & Large Scale MoE Disclosure with calculations; single endpoint is not law. \\
2024 & OLMo 2\cite{en:olmo2} & Dense & 1B; 7B; 13B; 32B & Same as total & Up to 5T--6T & B$+$ & Modern fully open family, suitable for data quality research. \\
2025 & Qwen3\cite{en:qwen3} & Dense+\allowbreak MoE & 0.6B; 1.7B; 4B; 8B; 14B; 32B; 30B-MoE; 235B-MoE & Dense is the same as total; MoE 3B/22B & is about 36T & B$+$ & The dense gradient of the same generation is compared with active-parameter MoE. \\
2025 & Llama 4\cite{en:llama4} & MoE & Scout 109B; Maverick 400B & Both 17B & Not disclosed as fit-ready & C$+$ & Multimodality is mixed with distillation and only supports active/total distinction. \\
2025 & Gemma 3\cite{en:gemma3} & Dense Multi-modal & 270M; 1B; 4B; 12B; 27B & Same as total & Varies with size & C$+$ & Deployment size gradient; multimodal versus recipe-limited classic comparison. \\
2025\newline 2026 & LongCat\cite{en:longcatflash,en:longcat2} & MoE & Flash 560B; 2.0 1.6T & Flash 18.6--31.3B (average approx. 27B); 2.0 about 48B & Flash undisclosed; 2.0 more than 35T & C$+$ & same family total/active/data simultaneous expansion; cross-generation architecture changes cannot be attributed to a single axis. \\
2025\newline 2026 & Kimi K2/K3\cite{en:kimik2,en:kimik3} & MoE Multi-mode & K2 1T; K3 2.8T & 32B; 104B & K2 15.5T; K3 undisclosed & C$+$ & shows higher sparsity capacity and active compute joint expansion; K3 is not fit-ready. \\
2025\newline 2026 & GLM 4.5/5\cite{en:glm5} & MoE & 355B; 744B & 32B; 40B & 23T; 28.5T & B$-$ & exposes vertical points in the same family; parameters, data, architecture and post-training change at the same time. \\
2026 & Gemma 4\cite{en:gemma4} & Dense+\allowbreak MoE Multi-mode & E2B; E4B; 12B; 31B; 26B-A4B & A4B About 4B active & Unpublished & C & Current inventory; lacks evidence of fitting scaling law. \\
2026 & DeepSeek-V4\cite{en:deepseekv4} & MoE & Flash 284B; Pro 1.6T & 13B; 49B & Undisclosed & C & total capacity increases while active compute is relatively restrained; not fit-ready. \\
2026 & Claude Fable 5\cite{en:claudefable5} & -- & -- & -- & -- & C & Official product and system card, but no disclosed training scale; useful only as an evidence boundary. \\
\end{longtable}
...
\begin{table}[htbp]
\centering
\caption{Scaling experiment minimum record field. }\label{en:tab:schema}
...
\bibitem{en:hestness2017} \bibentry{J. Hestness et al.}{Deep Learning Scaling is Predictable, Empirically}{arXiv:1712.00409}{2017}{https://arxiv.org/abs/1712.00409}
\bibitem{en:rosenfeld2019} \bibentry{J. S. Rosenfeld et al.}{A Constructive Prediction of the Generalization Error Across Scales}{ICLR}{2020}{https://arxiv.org/abs/1909.12673}
\bibitem{en:kaplan2020} \bibentry{J. Kaplan et al.}{Scaling Laws for Neural Language Models}{arXiv:2001.08361}{2020}{https://arxiv.org/abs/2001.08361}
\bibitem{en:bahri2021} \bibentry{Y. Bahri et al.}{Explaining Neural Scaling Laws}{JMLR}{2024}{https://arxiv.org/abs/2102.06701}
\bibitem{en:caballero2022} \bibentry{E. Caballero et al.}{Broken Neural Scaling Laws}{arXiv:2210.14891}{2022}{https://arxiv.org/abs/2210.14891}
\bibitem{en:brown2020} \bibentry{T. B. Brown et al.}{Language Models are Few-Shot Learners}{NeurIPS}{2020}{https://arxiv.org/abs/2005.14165}
\bibitem{en:hoffmann2022} \bibentry{J. Hoffmann et al.}{Training Compute-Optimal Large Language Models}{NeurIPS}{2022}{https://doi.org/10.48550/arXiv.2203.15556}
\bibitem{en:rae2021} \bibentry{J. W. Rae et al.}{Scaling Language Models: Methods, Analysis \& Insights from Training Gopher}{arXiv:2112.11446}{2021}{https://arxiv.org/abs/2112.11446}
\bibitem{en:besiroglu2024} \bibentry{T. Besiroglu et al.}{Chinchilla Scaling: A Replication Attempt}{arXiv:2404.10102}{2024}{https://arxiv.org/abs/2404.10102}
\bibitem{en:henighan2020} \bibentry{T. Henighan et al.}{Scaling Laws for Autoregressive Generative Modeling}{arXiv:2010.14701}{2020}{https://arxiv.org/abs/2010.14701}
\bibitem{en:hernandez2021} \bibentry{D. Hernandez et al.}{Scaling Laws for Transfer}{arXiv:2102.01293}{2021}{https://arxiv.org/abs/2102.01293}
\bibitem{en:openai2023gpt4} \bibentry{OpenAI}{GPT-4 Technical Report}{arXiv:2303.08774}{2023}{https://arxiv.org/abs/2303.08774}
\bibitem{en:wei2022} \bibentry{J. Wei et al.}{Emergent Abilities of Large Language Models}{TMLR}{2022}{https://arxiv.org/abs/2206.07682}
\bibitem{en:schaeffer2023} \bibentry{R. Schaeffer, B. Miranda, and S. Koyejo}{Are Emergent Abilities of Large Language Models a Mirage?}{NeurIPS}{2023}{https://arxiv.org/abs/2304.15004}
\bibitem{en:muennighoff2023} \bibentry{N. Muennighoff et al.}{Scaling Data-Constrained Language Models}{arXiv:2305.16264}{2023}{https://arxiv.org/abs/2305.16264}
\bibitem{en:deepseekllm2024} \bibentry{DeepSeek-AI et al.}{DeepSeek LLM: Scaling Open-Source Language Models with Longtermism}{arXiv:2401.02954}{2024}{https://arxiv.org/abs/2401.02954}
\bibitem{en:datacomplm2024} \bibentry{J. Li et al.}{DataComp-LM: In Search of the Next Generation of Training Sets for Language Models}{arXiv:2406.11794}{2024}{https://arxiv.org/abs/2406.11794}
\bibitem{en:datamixing2024} \bibentry{J. Ye et al.}{Data Mixing Laws: Optimizing Data Mixtures by Predicting Language Modeling Performance}{arXiv:2403.16952}{2024}{https://arxiv.org/abs/2403.16952}
\bibitem{en:sardana2024} \bibentry{N. Sardana et al.}{Beyond Chinchilla-Optimal: Accounting for Inference in Language Model Scaling Laws}{ICML}{2024}{https://arxiv.org/abs/2401.00448}
\bibitem{en:snell2024} \bibentry{C. Snell et al.}{Scaling LLM Test-Time Compute Optimally Can Be More Effective than Scaling Model Parameters}{arXiv:2408.03314}{2024}{https://arxiv.org/abs/2408.03314}
\bibitem{en:biderman2023} \bibentry{S. Biderman et al.}{Pythia: A Suite for Analyzing Large Language Models Across Training and Scaling}{ICML}{2023}{https://arxiv.org/abs/2304.01373}
\bibitem{en:cerebrasgpt2023} \bibentry{N. Dey et al.}{Cerebras-GPT: Open Compute-Optimal Language Models Trained on the Pile}{arXiv:2304.03208}{2023}{https://arxiv.org/abs/2304.03208}
\bibitem{en:llama2023} \bibentry{H. Touvron et al.}{LLaMA: Open and Efficient Foundation Language Models}{arXiv:2302.13971}{2023}{https://arxiv.org/abs/2302.13971}
\bibitem{en:llama2} \bibentry{H. Touvron et al.}{Llama 2: Open Foundation and Fine-Tuned Chat Models}{arXiv:2307.09288}{2023}{https://arxiv.org/abs/2307.09288}
\bibitem{en:palm2022} \bibentry{A. Chowdhery et al.}{PaLM: Scaling Language Modeling with Pathways}{arXiv:2204.02311}{2022}{https://arxiv.org/abs/2204.02311}
\bibitem{en:opt2022} \bibentry{S. Zhang et al.}{OPT: Open Pre-trained Transformer Language Models}{arXiv:2205.01068}{2022}{https://arxiv.org/abs/2205.01068}
\bibitem{en:bloom2022} \bibentry{T. L. Scao et al.}{BLOOM: A 176B-Parameter Open-Access Multilingual Language Model}{arXiv:2211.05100}{2022}{https://arxiv.org/abs/2211.05100}
\bibitem{en:falcon2023} \bibentry{E. Almazrouei et al.}{The Falcon Series of Open Language Models}{arXiv:2311.16867}{2023}{https://arxiv.org/abs/2311.16867}
\bibitem{en:mixtral2024} \bibentry{A. Q. Jiang et al.}{Mixtral of Experts}{arXiv:2401.04088}{2024}{https://arxiv.org/abs/2401.04088}
\bibitem{en:openelm2024} \bibentry{S. Mehta et al.}{OpenELM: An Efficient Language Model Family with Open Training and Inference Framework}{arXiv:2404.14619}{2024}{https://arxiv.org/abs/2404.14619}
\bibitem{en:olmo2024} \bibentry{D. Groeneveld et al.}{OLMo: Accelerating the Science of Language Models}{arXiv:2402.00838}{2024}{https://arxiv.org/abs/2402.00838}
\bibitem{en:llama31} \bibentry{Meta AI}{Introducing Llama 3.1: Our Most Capable Models to Date}{Official release}{2024}{https://ai.meta.com/blog/meta-llama-3-1/}
\bibitem{en:qwen25} \bibentry{Qwen Team}{Qwen2.5 Technical Report}{arXiv:2412.15115}{2024}{https://arxiv.org/abs/2412.15115}
\bibitem{en:qwen25coder} \bibentry{B. Hui et al.}{Qwen2.5-Coder Technical Report}{arXiv:2409.12186}{2024}{https://arxiv.org/abs/2409.12186}
\bibitem{en:gemma1} \bibentry{Gemma Team}{Gemma: Open Models Based on Gemini Research and Technology}{arXiv:2403.08295}{2024}{https://arxiv.org/abs/2403.08295}
\bibitem{en:gemma2} \bibentry{Gemma Team}{Gemma 2: Improving Open Language Models at a Practical Size}{arXiv:2408.00118}{2024}{https://arxiv.org/abs/2408.00118}
\bibitem{en:deepseekv2} \bibentry{DeepSeek-AI et al.}{DeepSeek-V2: A Strong, Economical, and Efficient Mixture-of-Experts Language Model}{arXiv:2405.04434}{2024}{https://arxiv.org/abs/2405.04434}
\bibitem{en:deepseekv3} \bibentry{DeepSeek-AI et al.}{DeepSeek-V3 Technical Report}{arXiv:2412.19437}{2024}{https://arxiv.org/abs/2412.19437}
\bibitem{en:olmo2} \bibentry{Allen Institute for AI}{OLMo 2: The Best Fully Open Language Model to Date}{Official release}{2024}{https://allenai.org/olmo2}
\bibitem{en:qwen3} \bibentry{Qwen Team}{Qwen3: Think Deeper, Act Faster}{Official technical blog}{2025}{https://qwenlm.github.io/blog/qwen3/}
\bibitem{en:llama4} \bibentry{Meta AI}{The Llama 4 Herd: The Beginning of a New Era of Natively Multimodal AI Innovation}{Official release}{2025}{https://ai.meta.com/blog/llama-4-multimodal-intelligence/}
\bibitem{en:gemma3} \bibentry{Google}{Gemma Models Documentation}{Official documentation; accessed 2026-08-07}{2025}{https://ai.google.dev/gemma/docs/get_started}
\bibitem{en:longcatflash} \bibentry{LongCat Team}{LongCat-Flash Technical Report}{Official repository and technical report}{2025}{https://github.com/meituan-longcat/LongCat-Flash-Chat}
\bibitem{en:longcat2} \bibentry{LongCat Team}{LongCat-2.0}{Official repository and technical blog; accessed 2026-08-07}{2026}{https://github.com/meituan-longcat/LongCat-2.0}
\bibitem{en:kimik2} \bibentry{Kimi Team et al.}{Kimi K2: Open Agentic Intelligence}{arXiv:2507.20534}{2025}{https://arxiv.org/abs/2507.20534}
\bibitem{en:kimik3} \bibentry{Kimi Team}{Kimi K3: Open Frontier Intelligence}{Official repository and technical report; accessed 2026-08-07}{2026}{https://github.com/MoonshotAI/Kimi-K3}
\bibitem{en:glm5} \bibentry{Z.ai}{GLM-5: From Vibe Coding to Agentic Engineering}{Official repository and technical report; accessed 2026-08-07}{2026}{https://github.com/zai-org/GLM-5}
\bibitem{en:gemma4} \bibentry{Google}{Gemma Models Documentation: Current Model Inventory}{Official documentation; accessed 2026-08-07}{2026}{https://ai.google.dev/gemma/docs/get_started}
\bibitem{en:deepseekv4} \bibentry{DeepSeek}{DeepSeek-V4 Official Release}{Official release}{2026}{https://api-docs.deepseek.com/news/news260424/}
\bibitem{en:claudefable5} \bibentry{Anthropic}{Claude Fable 5 \& Claude Mythos 5 System Card}{Official system card}{2026}{https://www-cdn.anthropic.com/2f9323abbcc4abe219577539efe19a623c9ca2bd/Claude\%20Fable\%205\%20\%26\%20Claude\%20Mythos\%205\%20System\%20Card.pdf}
...
\renewcommand{\reportshorttitle}{LLM Scaling Law 技术报告}
\markboth{}{}
\begin{center}
  {\sffamily\bfseries\zihao{1}\color{deepblue}
...
这四个问题的证据强度不同。连续、同分布且与训练目标一致的 validation loss 通常噪声最低；exact match、pass@1 等离散指标受到阈值、prompt、采样和污染影响；全生命周期最优又取决于调用量、显存、延迟和服务生命周期。因此，本文采用表~\ref{zh:tab:evidence-level} 的证据分级。

\begin{table}[htbp]
\centering
\caption{Scaling 证据分级。只有 A 级证据适合拟合规律，B/C 级主要用于趋势校验。}
\label{zh:tab:evidence-level}
\begin{tabularx}{\textwidth}{C{1.1cm}YY}
\toprule
等级 & 定义 & 典型材料 \\
\midrule
...
机器学习长期使用 learning curve 观察数据量与泛化误差的关系。现代神经 scaling law 的转折点是 Hestness 等人在机器翻译、语言建模、图像和语音等任务上观察到：在多个数量级内，可约误差常近似服从简单幂律，最优模型规模也随数据量规律增长\cite{zh:hestness2017}。Rosenfeld 等人进一步提出同时依赖模型规模和数据规模的函数形式，并用较小尺度实验预测更大尺度表现\cite{zh:rosenfeld2019}。

单变量幂律通常写为
\begin{equation}
  L(x)=L_{\infty}+A x^{-\alpha},
  \label{zh:eq:single-power}
\end{equation}
其中 $x$ 可以是参数量、数据量或计算量；$L_{\infty}$ 是当前分布与模型族下的渐近损失下限；$A x^{-\alpha}$ 是可通过扩展资源降低的部分；$\alpha>0$ 是扩展效率。减去 $L_{\infty}$ 后，式~\eqref{zh:eq:single-power} 在 log--log 坐标中成为直线：
\begin{equation}
  \log\!\left(L-L_{\infty}\right)=\log A-\alpha\log x.
  \label{zh:eq:loglinear}
\end{equation}
指数通常很小，意味着边际收益递减很强。更重要的是，$L_{\infty}$ 未知时不能直接做普通线性回归；否则斜率会产生系统偏差。

\subsection{Kaplan：统一参数、数据和计算三轴}

Kaplan 等人把语言模型的参数量 $N$、训练 token 数 $D$ 与训练计算量 $C$ 放入统一框架，跨多个数量级拟合 loss 曲线\cite{zh:kaplan2020}。其代表性指数约为 $\alpha_N=$\textbf{0.076}、$\alpha_D=$\textbf{0.095}、$\alpha_C=$\textbf{0.050}，并得到早期 compute-optimal 结论：预算增长时模型规模应比数据量增长得更快，近似为
\begin{equation}
  N_{\mathrm{opt}}\propto C^{0.73},\qquad
  D_{\mathrm{opt}}\propto C^{0.27}.
  \label{zh:eq:kaplan-opt}
\end{equation}
这一结论影响了 GPT-3 式“大模型、相对少 token、较早停止”的训练路线\cite{zh:brown2020}。它的贡献不在于给出永恒指数，而在于把“扩大模型是否有效”变成了“可用 pilot 预测目标 run”的工程问题。

\subsection{为什么幂律有用，却不是自然定律}

幂律可以来自不同 regime 下的方差限制、分辨率限制或逼近误差；理论工作能够解释常见的函数形态，但仍不能给出跨数据分布与架构通用的指数\cite{zh:bahri2021}。经验上还会出现 plateau、double descent、迟发跃迁或 slope change，smoothly broken power law 往往比单一幂律更合适\cite{zh:caballero2022}。因此需要把式~\eqref{zh:eq:single-power} 理解为\term{有限区间、固定 recipe 下的局部模型}。

\section{核心数学：从联合损失面到 Compute-Optimal}

\subsection{变量口径与训练计算近似}

表~\ref{zh:tab:symbols} 给出本文统一口径。对于标准 dense decoder-only Transformer，常用一阶近似
\begin{equation}
  C\approx 6ND.
  \label{zh:eq:compute}
\end{equation}
...
Hoffmann 等人训练了 400 多个、70M--16B 参数、5B--500B token 的模型，采用联合损失面\cite{zh:hoffmann2022}：
\begin{equation}
  L(N,D)=E+\frac{A}{N^{\alpha}}+\frac{B}{D^{\beta}},
  \qquad A,B,\alpha,\beta>0.
  \label{zh:eq:chinchilla}
\end{equation}
在式~\eqref{zh:eq:compute} 的计算约束下，令 $D=C/(6N)$，代回式~\eqref{zh:eq:chinchilla}：
\begin{equation}
  L(N\mid C)=E+A N^{-\alpha}
  +B\left(\frac{6N}{C}\right)^{\beta}.
  \label{zh:eq:constrained-loss}
\end{equation}
令式~\eqref{zh:eq:constrained-loss} 对 $N$ 的导数为零，可得两个边际收益平衡，并得到
\begin{align}
  N_{\mathrm{opt}}(C)
  &=G\left(\frac{C}{6}\right)^{\frac{\beta}{\alpha+\beta}},
  &G&=\left(\frac{\alpha A}{\beta B}\right)^{\frac{1}{\alpha+\beta}},
...
\label{zh:tab:kaplan-chinchilla}
\begin{tabularx}{\textwidth}{Y C{3.2cm} C{3.2cm}}
\toprule
...
Chinchilla 以与 Gopher 近似的训练计算量，把 280B 参数、300B token 改为 70B 参数、1.4T token，并在其报告的多数任务上更好\cite{zh:rae2021,zh:hoffmann2022}。这使 scaling law 从“解释曲线”变成了“直接改变训练配方”的工具。

“约 \textbf{20 tokens/parameter}”只是特定数据、模型族和实验区间下的工程口径。它优化单次训练 FLOPs，不包含推理成本、显存、数据质量与重复。Besiroglu 等人的复现还发现 Chinchilla Approach 3 的拟合存在优化器提前停止和置信区间过窄问题；重新拟合的点估计仍与约 \textbf{20--26 tokens/parameter} 相容，但\textit{合理区间明显更宽}\cite{zh:besiroglu2024}。稳健结论是\term{参数和 token 应共同扩展}，而不是精确常数可跨体系迁移。

\section{技术演进：Scaling 的自变量不断增加}

\subsection{迁移、多模态与生产级预测}

Henighan 等人把幂律扩展到图像、视频、多模态和数学问题生成\cite{zh:henighan2020}；Hernandez 等人把预训练收益解释为下游数据的“有效倍增”，并刻画 transfer scaling\cite{zh:hernandez2021}。GPT-4 技术报告展示了生产级用途：用计算量低至四个数量级的小模型预测最终内部 codebase loss，并以低至三个数量级的模型预测 HumanEval 子集表现；报告同时明确部分能力仍难预测\cite{zh:openai2023gpt4}。这支持的是\term{受控训练体系内的可预测性}，不是任意能力的普适预测。

\subsection{从平滑 Loss 到“涌现”争论}

“Emergent abilities”把小模型近乎为零、大模型突然上升的 benchmark 能力解释为跃迁\cite{zh:wei2022}。但不连续指标本身就能制造跳变：将 exact match 换为 token-level accuracy 或 Brier score 后，很多曲线会更平滑\cite{zh:schaeffer2023}。因此至少要区分：模型内部能力可能平滑增长；任务成功率可能因阈值呈 S 型；prompt 与采样制造高方差；三个模型点不足以证明 phase transition。

\subsection{从“更多数据”到“有效数据”}

Data-constrained scaling 研究表明，在固定计算预算下适度重复数据可能损失很小，但重复的边际价值最终趋近于零\cite{zh:muennighoff2023}。DeepSeek LLM 报告不同数据版本会改变最优分配指数，说明 scaling coefficient 本身可作为数据与 recipe 的体系内诊断量\cite{zh:deepseekllm2024}。DataComp-LM 用标准化小模型实验展示过滤策略能显著移动性能前沿\cite{zh:datacomplm2024}；data mixing law 则把 mixture proportion 也作为可拟合变量\cite{zh:datamixing2024}。

把这些结果统一起来，可以写成更现实但不可直接一次拟合的关系：
\begin{equation}
 \mathrm{quality}=f\!\left(N,D,Q_{\mathrm{data}},C_{\mathrm{train}},
 C_{\mathrm{test}},\mathrm{architecture},\mathrm{objective}\right).
 \label{zh:eq:modern-scaling}
\end{equation}
...
\begin{equation}
  J=C_{\mathrm{train}}+R\,c_{\mathrm{infer}}(N,C_{\mathrm{test}}),
  \label{zh:eq:lifecycle}
\end{equation}
而不是只最小化训练 FLOPs。Inference-aware scaling 的结果是：\textbf{高调用量下，训练一个更小但看过更多 token 的模型可能拥有更低总成本}\cite{zh:sardana2024}。Test-time scaling 进一步表明，\textit{自适应分配搜索或 verifier compute，在部分问题上可以比单纯扩大参数更有效}\cite{zh:snell2024}。现代 scaling 因而从 $N$--$D$--$C_{\mathrm{train}}$ 三角形扩展到训练、部署与推理时计算的联合优化。
...
表~\ref{zh:tab:experiment-matrix} 是面向 7B 目标模型的最小可用设计。规模比绝对数更重要：每条 isoFLOP 至少覆盖最优点两侧，最大 pilot 与目标点之间不宜跨越过多数量级。

\begin{table}[htbp]
\centering
\caption{建议的 pilot 实验矩阵。每个格点代表一个完整 run，不是同一 run 的 checkpoint。}
\label{zh:tab:experiment-matrix}
\begin{tabularx}{\textwidth}{Y Y Y Y}
\toprule
维度 & 推荐取值 & 目的 & 最低控制要求 \\
\midrule
...
附录~\ref{zh:app:model-inventory} 收录 2020--2026 年 32 个公开模型家族。主文只抽取适合形成判断的对照。表~\ref{zh:tab:dense-families} 中，GPT-3、Pythia、Cerebras-GPT 尺寸点密集；LLaMA、Llama 3.1、Qwen2.5/Qwen3 与 OLMo 2 更能代表现代 data-rich 与 deployment-oriented 训练。LongCat、Kimi 与 GLM 的跨代公开数字则提供了较新的 MoE 纵向对照；Claude Fable 5 的参数和训练 token 未披露，只能用于标示闭源 frontier 的证据边界。

\begin{table}[htbp]
\centering
\small
\caption{具有代表性的 dense 模型家族及其证据边界。}
\label{zh:tab:dense-families}
\begin{tabularx}{\textwidth}{p{2.2cm}p{3.9cm}p{2.5cm}Y}
\toprule
家族 & 尺寸梯度 & 训练 token & 能支持的判断 \\
\midrule
GPT-3 & 125M--175B，共 8 点 & 每个约 300B & 同 recipe 的 size/few-shot 趋势；不适合估 compute-optimal ratio\cite{zh:brown2020} \\
Pythia & 70M--12B，共 8 点 & 每个约 300B & 开放 checkpoints 与固定数据顺序，适合训练动力学\cite{zh:biderman2023} \\
Cerebras-GPT & 111M--13B，共 7 点 & 20 tokens/param. & 公开 Chinchilla-style family\cite{zh:cerebrasgpt2023} \\
LLaMA & 7B, 13B, 33B, 65B & 1.0T/1.4T & data-rich 小模型可超过旧大模型；token budget 不同\cite{zh:llama2023} \\
Llama 3.1 & 8B, 70B, 405B & 超过 15T & 现代家族远超 20 tokens/parameter；不可与旧代连线\cite{zh:llama31} \\
Qwen2.5 & 0.5B--72B，共 7 点 & 18T & 现代 dense 尺寸梯度，说明数据/recipe 移动 frontier\cite{zh:qwen25} \\
Qwen3 Dense & 0.6B--32B，共 6 点 & 约 36T & 新代小模型追平旧代大模型是曲线整体移动，不是参数失效\cite{zh:qwen3} \\
OLMo 2 & 1B, 7B, 13B, 32B & 最高 5T--6T & 数据、代码、recipe 与 checkpoints 可审计\cite{zh:olmo2} \\
\bottomrule
\end{tabularx}
\end{table}
...
表~\ref{zh:tab:moe} 展示如果只看 total parameters，会如何误读 MoE。总参数更接近可存储的专家容量，active parameters 更接近单 token 的前向计算，但 routing、共享专家、通信与负载均衡仍不可忽略。
...
\midrule
Mixtral 8x7B & 46.7B & 12.9B & 未公开 & 存储容量近 47B，单 token 计算近 13B\cite{zh:mixtral2024} \\
DeepSeek-V2 & 236B & 21B & 8.1T & 大 total capacity 与低 active compute\cite{zh:deepseekv2} \\
DeepSeek-V3 & 671B & 37B & 14.8T & 不能将 671B 直接代入 dense 式~\eqref{zh:eq:compute}\cite{zh:deepseekv3} \\
Qwen3-30B-A3B & 30B & 3B & 约 36T & 小 active 模型借专家容量提升效果\cite{zh:qwen3} \\
Qwen3-235B-A22B & 235B & 22B & 约 36T & total/active 共同决定容量与成本\cite{zh:qwen3} \\
Llama 4 Scout/Maverick & 109B/400B & 17B/17B & 未公开 & 多模态与 distillation 混杂，不是 clean scaling suite\cite{zh:llama4} \\
DeepSeek-V4 Flash/Pro & 284B/1.6T & 13B/49B & 未公开 & 2026 endpoint，只支持架构趋势观察\cite{zh:deepseekv4} \\
\bottomrule
\end{tabularx}
\end{table}
...
\clearpage
\section{术语速查}
\label{zh:app:glossary}

\begin{table}[htbp]
\centering
\caption{Scaling law 常用术语。}\label{zh:tab:glossary}
\begin{tabularx}{\textwidth}{p{3.6cm}X}
\toprule
术语 & 定义 \\
\midrule
...
\label{zh:app:model-inventory}

表~\ref{zh:tab:model-inventory} 的 token 数来自对应论文或官方发布页。“未公开”表示没有 fit-ready 的公开数字，并非推断为零；open weights 不等同于训练数据与过程完全开放。若 token budget 随尺寸变化，应回到原报告逐模型展开。2026 年模型只在官方资料明确给出尺寸时纳入，并统一标为 C 级趋势证据。
...
\setlength{\LTright}{0pt}
\noindent\begin{minipage}{\textwidth}
\captionof{table}{2020--2026 年公开 LLM 家族、尺寸、训练 token 与 scaling 证据。}\label{zh:tab:model-inventory}
\end{minipage}
\begin{longtable}{C{0.55cm}p{1.50cm}p{1.10cm}p{3.20cm}p{2.00cm}p{1.80cm}C{0.65cm}p{3.80cm}}
\toprule
...
\bottomrule
\endlastfoot
2020 & GPT-3\cite{zh:brown2020} & Dense & 125M; 350M; 760M; 1.3B; 2.7B; 6.7B; 13B; 175B & 同 total & 每个约 300B & A & 同家族参数 scaling 与 few-shot 趋势；不能估 compute-optimal ratio。 \\
2021 & Gopher\cite{zh:rae2021} & Dense & 44M; 117M; 417M; 1.4B; 7.1B; 280B & 同 total & 主模型约 300B & A$-$ & 尺寸 scaling 与不同任务收益不均衡。 \\
2022 & Chinchilla\cite{zh:hoffmann2022} & Dense & 70B & 同 total & 1.4T & A & 与 Gopher 的同计算对照及 compute-optimal allocation。 \\
2022 & PaLM\cite{zh:palm2022} & Dense & 8B; 62B; 540B & 同 total & 780B & A$-$ & 家族内尺寸趋势与 apparent benchmark thresholds。 \\
2022 & OPT\cite{zh:opt2022} & Dense & 125M; 350M; 1.3B; 2.7B; 6.7B; 13B; 30B; 66B; 175B & 同 total & 主设置约 180B & B$+$ & 尺寸覆盖广；须结合 recipe 与 loss 控制。 \\
2022 & BLOOM\cite{zh:bloom2022} & Dense & 560M; 1.1B; 1.7B; 3B; 7.1B; 176B & 同 total & 176B 模型约 350B & B & 多语言规模观察；尺寸梯度有缺口。 \\
2023 & Pythia\cite{zh:biderman2023} & Dense & 70M; 160M; 410M; 1B; 1.4B; 2.8B; 6.9B; 12B & 同 total & 每个约 300B & A$+$ & 最佳公开受控 suite 之一，适合训练动力学。 \\
2023 & Cerebras-GPT\cite{zh:cerebrasgpt2023} & Dense & 111M; 256M; 590M; 1.3B; 2.7B; 6.7B; 13B & 同 total & 20 tokens/parameter & A & 公开 Chinchilla-style family。 \\
2023 & LLaMA\cite{zh:llama2023} & Dense & 7B; 13B; 33B; 65B & 同 total & 1.0T 或 1.4T & B$+$ & data-rich 小模型可超过旧大模型；不是单轴拟合。 \\
2023 & Llama 2\cite{zh:llama2} & Dense & 7B; 13B; 70B & 同 total & 2T & B$+$ & 同代部署梯度；点数不足以稳健拟合指数。 \\
2023 & Falcon\cite{zh:falcon2023} & Dense & 7B; 40B; 180B & 同 total & 180B 最高约 3.5T & B & data-rich 与 web filtering；token budget 随尺寸变化。 \\
2023 & DeepSeek LLM\cite{zh:deepseekllm2024} & Dense & 7B; 67B & 同 total & 2T & A$-$ & pilot 外推，并显示数据版本会改变拟合指数。 \\
2023 & Mixtral 8x7B\cite{zh:mixtral2024} & MoE & 46.7B & 12.9B & 未公开 & C$+$ & total 与 active compute 必须分开；不是 scaling fit。 \\
2024 & Open\allowbreak ELM\cite{zh:openelm2024} & Dense & 270M; 450M; 1.1B; 3B & 同 total & 1.5T & B$+$ & 同 token budget 的小模型架构比较。 \\
2024 & OLMo\cite{zh:olmo2024} & Dense & 1B; 7B & 同 total & 约 2T--3T & B$+$ & 数据顺序、checkpoints 与训练日志可审计。 \\
2024 & Llama 3.1\cite{zh:llama31} & Dense & 8B; 70B; 405B & 同 total & 超过 15T & B$+$ & 现代 data-rich family；不支持 universal coefficients。 \\
2024 & Qwen2.5\cite{zh:qwen25} & Dense & 0.5B; 1.5B; 3B; 7B; 14B; 32B; 72B & 同 total & 18T & B$+$ & 现代尺寸梯度，显示 data/recipe 移动 frontier。 \\
2024 & Qwen2.5-Coder\cite{zh:qwen25coder} & Dense & 0.5B; 1.5B; 3B; 7B; 14B; 32B & 同 total & 续训语料 5.5T & B$+$ & 代码域梯度；需区分 base pretraining 与 CPT。 \\
2024 & Gemma 1\cite{zh:gemma1} & Dense & 2B; 7B & 同 total & 2T; 6T & B & 两个点；说明部署家族不固定 tokens/parameter。 \\
2024 & Gemma 2\cite{zh:gemma2} & Dense & 2B; 9B; 27B & 同 total & 2T; 8T; 13T & C$+$ & 2B/9B 含 distillation，反例说明不能只看参数。 \\
2024 & DeepSeek-V2\cite{zh:deepseekv2} & MoE & 236B & 21B & 8.1T & B & 分离存储容量与 per-token compute。 \\
2024 & DeepSeek-V3\cite{zh:deepseekv3} & MoE & 671B & 37B & 14.8T & B & 大规模 MoE 与计算披露；单 endpoint 不是 law。 \\
2024 & OLMo 2\cite{zh:olmo2} & Dense & 1B; 7B; 13B; 32B & 同 total & 最高 5T--6T & B$+$ & 现代 fully open family，适合数据质量研究。 \\
2025 & Qwen3\cite{zh:qwen3} & Dense+\allowbreak MoE & 0.6B; 1.7B; 4B; 8B; 14B; 32B; 30B-MoE; 235B-MoE & Dense 同 total; MoE 3B/22B & 约 36T & B$+$ & 同代 dense 梯度与 active-parameter MoE 对照。 \\
2025 & Llama 4\cite{zh:llama4} & MoE & Scout 109B; Maverick 400B & 均 17B & 未以 fit-ready 形式披露 & C$+$ & 多模态与 distillation 混杂，只支持 active/total 区分。 \\
2025 & Gemma 3\cite{zh:gemma3} & Dense 多模态 & 270M; 1B; 4B; 12B; 27B & 同 total & 随尺寸变化 & C$+$ & 部署尺寸梯度；多模态与 recipe 限制经典比较。 \\
2025\newline 2026 & LongCat\cite{zh:longcatflash,zh:longcat2} & MoE & Flash 560B; 2.0 1.6T & Flash 18.6--31.3B（均值约 27B）; 2.0 约 48B & Flash 未公开; 2.0 超过 35T & C$+$ & 同家族 total/active/data 同时扩展；跨代架构变化，不能归因于单轴。 \\
2025\newline 2026 & Kimi K2/K3\cite{zh:kimik2,zh:kimik3} & MoE 多模态 & K2 1T; K3 2.8T & 32B; 104B & K2 15.5T; K3 未公开 & C$+$ & 展示更高 sparsity 下容量与 active compute 联合扩展；K3 非 fit-ready。 \\
2025\newline 2026 & GLM 4.5/5\cite{zh:glm5} & MoE & 355B; 744B & 32B; 40B & 23T; 28.5T & B$-$ & 同家族公开纵向点；参数、数据、架构及 post-training 同时变化。 \\
2026 & Gemma 4\cite{zh:gemma4} & Dense+\allowbreak MoE 多模态 & E2B; E4B; 12B; 31B; 26B-A4B & A4B 约 4B active & 未公开 & C & 当前 inventory；缺乏拟合 scaling law 的证据。 \\
2026 & DeepSeek-V4\cite{zh:deepseekv4} & MoE & Flash 284B; Pro 1.6T & 13B; 49B & 未公开 & C & total capacity 增长而 active compute 相对克制；非 fit-ready。 \\
2026 & Claude Fable 5\cite{zh:claudefable5} & 未公开 & 未公开 & 未公开 & 未公开 & C & 官方确认产品与 system card，但训练尺度未披露；只能作为披露边界，不能拟合 scaling。 \\
\end{longtable}
...
\begin{table}[htbp]
\centering
\caption{Scaling experiment 最低记录字段。}\label{zh:tab:schema}
\begin{tabularx}{\textwidth}{p{4.0cm}X}
\toprule
字段组 & 必填内容 \\
\midrule
...
\bibitem{zh:hestness2017} \bibentry{J. Hestness et al.}{Deep Learning Scaling is Predictable, Empirically}{arXiv:1712.00409}{2017}{https://arxiv.org/abs/1712.00409}
\bibitem{zh:rosenfeld2019} \bibentry{J. S. Rosenfeld et al.}{A Constructive Prediction of the Generalization Error Across Scales}{ICLR}{2020}{https://arxiv.org/abs/1909.12673}
\bibitem{zh:kaplan2020} \bibentry{J. Kaplan et al.}{Scaling Laws for Neural Language Models}{arXiv:2001.08361}{2020}{https://arxiv.org/abs/2001.08361}
\bibitem{zh:bahri2021} \bibentry{Y. Bahri et al.}{Explaining Neural Scaling Laws}{JMLR}{2024}{https://arxiv.org/abs/2102.06701}
\bibitem{zh:caballero2022} \bibentry{E. Caballero et al.}{Broken Neural Scaling Laws}{arXiv:2210.14891}{2022}{https://arxiv.org/abs/2210.14891}
\bibitem{zh:brown2020} \bibentry{T. B. Brown et al.}{Language Models are Few-Shot Learners}{NeurIPS}{2020}{https://arxiv.org/abs/2005.14165}
\bibitem{zh:hoffmann2022} \bibentry{J. Hoffmann et al.}{Training Compute-Optimal Large Language Models}{NeurIPS}{2022}{https://doi.org/10.48550/arXiv.2203.15556}
\bibitem{zh:rae2021} \bibentry{J. W. Rae et al.}{Scaling Language Models: Methods, Analysis \& Insights from Training Gopher}{arXiv:2112.11446}{2021}{https://arxiv.org/abs/2112.11446}
\bibitem{zh:besiroglu2024} \bibentry{T. Besiroglu et al.}{Chinchilla Scaling: A Replication Attempt}{arXiv:2404.10102}{2024}{https://arxiv.org/abs/2404.10102}
\bibitem{zh:henighan2020} \bibentry{T. Henighan et al.}{Scaling Laws for Autoregressive Generative Modeling}{arXiv:2010.14701}{2020}{https://arxiv.org/abs/2010.14701}
\bibitem{zh:hernandez2021} \bibentry{D. Hernandez et al.}{Scaling Laws for Transfer}{arXiv:2102.01293}{2021}{https://arxiv.org/abs/2102.01293}
\bibitem{zh:openai2023gpt4} \bibentry{OpenAI}{GPT-4 Technical Report}{arXiv:2303.08774}{2023}{https://arxiv.org/abs/2303.08774}
\bibitem{zh:wei2022} \bibentry{J. Wei et al.}{Emergent Abilities of Large Language Models}{TMLR}{2022}{https://arxiv.org/abs/2206.07682}
\bibitem{zh:schaeffer2023} \bibentry{R. Schaeffer, B. Miranda, and S. Koyejo}{Are Emergent Abilities of Large Language Models a Mirage?}{NeurIPS}{2023}{https://arxiv.org/abs/2304.15004}
\bibitem{zh:muennighoff2023} \bibentry{N. Muennighoff et al.}{Scaling Data-Constrained Language Models}{arXiv:2305.16264}{2023}{https://arxiv.org/abs/2305.16264}
\bibitem{zh:deepseekllm2024} \bibentry{DeepSeek-AI et al.}{DeepSeek LLM: Scaling Open-Source Language Models with Longtermism}{arXiv:2401.02954}{2024}{https://arxiv.org/abs/2401.02954}
\bibitem{zh:datacomplm2024} \bibentry{J. Li et al.}{DataComp-LM: In Search of the Next Generation of Training Sets for Language Models}{arXiv:2406.11794}{2024}{https://arxiv.org/abs/2406.11794}
\bibitem{zh:datamixing2024} \bibentry{J. Ye et al.}{Data Mixing Laws: Optimizing Data Mixtures by Predicting Language Modeling Performance}{arXiv:2403.16952}{2024}{https://arxiv.org/abs/2403.16952}
\bibitem{zh:sardana2024} \bibentry{N. Sardana et al.}{Beyond Chinchilla-Optimal: Accounting for Inference in Language Model Scaling Laws}{ICML}{2024}{https://arxiv.org/abs/2401.00448}
\bibitem{zh:snell2024} \bibentry{C. Snell et al.}{Scaling LLM Test-Time Compute Optimally Can Be More Effective than Scaling Model Parameters}{arXiv:2408.03314}{2024}{https://arxiv.org/abs/2408.03314}
\bibitem{zh:biderman2023} \bibentry{S. Biderman et al.}{Pythia: A Suite for Analyzing Large Language Models Across Training and Scaling}{ICML}{2023}{https://arxiv.org/abs/2304.01373}
\bibitem{zh:cerebrasgpt2023} \bibentry{N. Dey et al.}{Cerebras-GPT: Open Compute-Optimal Language Models Trained on the Pile}{arXiv:2304.03208}{2023}{https://arxiv.org/abs/2304.03208}
\bibitem{zh:llama2023} \bibentry{H. Touvron et al.}{LLaMA: Open and Efficient Foundation Language Models}{arXiv:2302.13971}{2023}{https://arxiv.org/abs/2302.13971}
\bibitem{zh:llama2} \bibentry{H. Touvron et al.}{Llama 2: Open Foundation and Fine-Tuned Chat Models}{arXiv:2307.09288}{2023}{https://arxiv.org/abs/2307.09288}
\bibitem{zh:palm2022} \bibentry{A. Chowdhery et al.}{PaLM: Scaling Language Modeling with Pathways}{arXiv:2204.02311}{2022}{https://arxiv.org/abs/2204.02311}
\bibitem{zh:opt2022} \bibentry{S. Zhang et al.}{OPT: Open Pre-trained Transformer Language Models}{arXiv:2205.01068}{2022}{https://arxiv.org/abs/2205.01068}
\bibitem{zh:bloom2022} \bibentry{T. L. Scao et al.}{BLOOM: A 176B-Parameter Open-Access Multilingual Language Model}{arXiv:2211.05100}{2022}{https://arxiv.org/abs/2211.05100}
\bibitem{zh:falcon2023} \bibentry{E. Almazrouei et al.}{The Falcon Series of Open Language Models}{arXiv:2311.16867}{2023}{https://arxiv.org/abs/2311.16867}
\bibitem{zh:mixtral2024} \bibentry{A. Q. Jiang et al.}{Mixtral of Experts}{arXiv:2401.04088}{2024}{https://arxiv.org/abs/2401.04088}
\bibitem{zh:openelm2024} \bibentry{S. Mehta et al.}{OpenELM: An Efficient Language Model Family with Open Training and Inference Framework}{arXiv:2404.14619}{2024}{https://arxiv.org/abs/2404.14619}
\bibitem{zh:olmo2024} \bibentry{D. Groeneveld et al.}{OLMo: Accelerating the Science of Language Models}{arXiv:2402.00838}{2024}{https://arxiv.org/abs/2402.00838}
\bibitem{zh:llama31} \bibentry{Meta AI}{Introducing Llama 3.1: Our Most Capable Models to Date}{Official release}{2024}{https://ai.meta.com/blog/meta-llama-3-1/}
\bibitem{zh:qwen25} \bibentry{Qwen Team}{Qwen2.5 Technical Report}{arXiv:2412.15115}{2024}{https://arxiv.org/abs/2412.15115}
\bibitem{zh:qwen25coder} \bibentry{B. Hui et al.}{Qwen2.5-Coder Technical Report}{arXiv:2409.12186}{2024}{https://arxiv.org/abs/2409.12186}
\bibitem{zh:gemma1} \bibentry{Gemma Team}{Gemma: Open Models Based on Gemini Research and Technology}{arXiv:2403.08295}{2024}{https://arxiv.org/abs/2403.08295}
\bibitem{zh:gemma2} \bibentry{Gemma Team}{Gemma 2: Improving Open Language Models at a Practical Size}{arXiv:2408.00118}{2024}{https://arxiv.org/abs/2408.00118}
\bibitem{zh:deepseekv2} \bibentry{DeepSeek-AI et al.}{DeepSeek-V2: A Strong, Economical, and Efficient Mixture-of-Experts Language Model}{arXiv:2405.04434}{2024}{https://arxiv.org/abs/2405.04434}
\bibitem{zh:deepseekv3} \bibentry{DeepSeek-AI et al.}{DeepSeek-V3 Technical Report}{arXiv:2412.19437}{2024}{https://arxiv.org/abs/2412.19437}
\bibitem{zh:olmo2} \bibentry{Allen Institute for AI}{OLMo 2: The Best Fully Open Language Model to Date}{Official release}{2024}{https://allenai.org/olmo2}
\bibitem{zh:qwen3} \bibentry{Qwen Team}{Qwen3: Think Deeper, Act Faster}{Official technical blog}{2025}{https://qwenlm.github.io/blog/qwen3/}
\bibitem{zh:llama4} \bibentry{Meta AI}{The Llama 4 Herd: The Beginning of a New Era of Natively Multimodal AI Innovation}{Official release}{2025}{https://ai.meta.com/blog/llama-4-multimodal-intelligence/}
\bibitem{zh:gemma3} \bibentry{Google}{Gemma Models Documentation}{Official documentation; accessed 2026-08-07}{2025}{https://ai.google.dev/gemma/docs/get_started}
\bibitem{zh:longcatflash} \bibentry{LongCat Team}{LongCat-Flash Technical Report}{Official repository and technical report}{2025}{https://github.com/meituan-longcat/LongCat-Flash-Chat}
\bibitem{zh:longcat2} \bibentry{LongCat Team}{LongCat-2.0}{Official repository and technical blog; accessed 2026-08-07}{2026}{https://github.com/meituan-longcat/LongCat-2.0}
\bibitem{zh:kimik2} \bibentry{Kimi Team et al.}{Kimi K2: Open Agentic Intelligence}{arXiv:2507.20534}{2025}{https://arxiv.org/abs/2507.20534}
\bibitem{zh:kimik3} \bibentry{Kimi Team}{Kimi K3: Open Frontier Intelligence}{Official repository and technical report; accessed 2026-08-07}{2026}{https://github.com/MoonshotAI/Kimi-K3}
\bibitem{zh:glm5} \bibentry{Z.ai}{GLM-5: From Vibe Coding to Agentic Engineering}{Official repository and technical report; accessed 2026-08-07}{2026}{https://github.com/zai-org/GLM-5}
\bibitem{zh:gemma4} \bibentry{Google}{Gemma Models Documentation: Current Model Inventory}{Official documentation; accessed 2026-08-07}{2026}{https://ai.google.dev/gemma/docs/get_started}
\bibitem{zh:deepseekv4} \bibentry{DeepSeek}{DeepSeek-V4 Official Release}{Official release}{2026}{https://api-docs.deepseek.com/news/news260424/}
\bibitem{zh:claudefable5} \bibentry{Anthropic}{Claude Fable 5 \& Claude Mythos 5 System Card}{Official system card}{2026}{https://www-cdn.anthropic.com/2f9323abbcc4abe219577539efe19a623c9ca2bd/Claude\%20Fable\%205\%20\%26\%20Claude\%20Mythos\%205\%20System\%20Card.pdf}
